...
def generate_forensic_pdf(
    ...
    agency = safe_val(case_info.get("agency"), "ForensiVault Digital Forensics Laboratory")
    ...
    ev_size = format_bytes(ev.get("total_bytes") or ev.get("size_bytes") or ev.get("size"))
    ...
    total_detected = stats.get("total_detected", len(recovered_files or []))
    total_recovered = stats.get("total_recovered", len([f for f in (recovered_files or []) if f.get("is_valid", True)]))
    total_partial = stats.get("total_partial", len([f for f in (recovered_files or []) if not f.get("is_valid", True)]))
    total_failed = stats.get("total_failed", 0)
    fragment_candidates = stats.get("fragment_candidates", 0)
    ...
    stats_data = [
        [
            ...
            Paragraph("Failed / Corrupted", bold_label_style),
        ...
        [
            ...
            Paragraph(str(total_failed), ParagraphStyle("s4", parent=body_style, fontName="Helvetica-Bold", fontSize=11, textColor=colors.HexColor("#C53030"), alignment=1)),
    ...
    files_headers = [
        ...
        Paragraph("Offset / Sector", bold_label_style),
        Paragraph("Method", bold_label_style),
        Paragraph("Confidence", bold_label_style),
    ...
    if not files_list:
        files_table_rows.append([
            Paragraph("No recovered files recorded in this case repository.", body_style),
        ...
        for f in files_list:
            ...
            fsize = format_bytes(f.get("size_bytes") or f.get("size"))
            ...
            offset_str = safe_val(offset_val)
            ...
            conf_str = f"{float(conf_val):.0f}%" if conf_val is not None else "Not available"
    ...
    files_table = Table(files_table_rows, colWidths=[90, 42, 62, 70, 80, 56, 104])
    ...
    if not audit_list:
        audit_table_rows.append([
            Paragraph("1", mono_style),
            Paragraph(gen_time, body_style),
            Paragraph("REPORT_GENERATION", body_style),
            Paragraph(examiner, body_style),
            Paragraph(case_id, mono_style),
            Paragraph("VERIFIED", ParagraphStyle("v", parent=body_style, fontName="Helvetica-Bold", textColor=colors.HexColor("#2E7D32")))
        ])
    else:
        for idx, a in enumerate(audit_list[:12], 1):
            eid = safe_val(a.get("entry_id") or a.get("id"), str(idx))
            ts = safe_val(a.get("timestamp"), gen_time)
            op = safe_val(a.get("operation_type") or a.get("action") or a.get("event_type"), "SYSTEM_OPERATION")
            ...
            status_color = colors.HexColor("#2E7D32") if "SUCCESS" in status or "COMPLETED" in status or "VERIFIED" in status else colors.HexColor("#C53030")
