    ...
    for (const auto& footerCandidate : eligible) {
        ...
        if (val.isValid) {
            ...
            if (conf.confidenceScore >= 80) {
                ...
                result.sha256 = core::BinaryUtils::sha256(result.reconstructedData);
                result.uncertaintyReason = "Successfully reconstructed from 2 disjoint fragments (Offsets: 0x" +
                                          core::BinaryUtils::byteToHex(headerFrag.offset >> 8) + ", 0x" +
                                          core::BinaryUtils::byteToHex(footerCandidate.offset >> 8) +
